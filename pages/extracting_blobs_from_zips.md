...
```
where `path/to/` is the path to freshly extracted payload.bin. Note that if you are using an Intel Mac, then you will have to change the path from `linux-x86` to `darwin-x86`.
